            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                switch (selectedIp) {
                    case DEFAULT_IP:
                        ...
                        break;
                    case ALTERNATE_IP:
                        if (!ipAddressEditText.getText().toString().startsWith(ALTERNATE_IP)) {
                            ipAddressEditText.setText(ALTERNATE_IP);
                        }
                        ipAddressEditText.setEnabled(true);
                        break;
                    case CUSTOM_IP:
                        if (ipAddressEditText.getText().toString().equals(DEFAULT_IP)) {
    ...
    private void setupIpAddressEditText() {
        ipAddressEditText.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                String enteredIp = s.toString();
                int selectedPosition = ipAddressSpinner.getSelectedItemPosition();

                if (selectedPosition == 1 && !enteredIp.startsWith(ALTERNATE_IP)) {
                    ipAddressEditText.setText(ALTERNATE_IP + enteredIp);
                    ipAddressEditText.setSelection(ipAddressEditText.getText().length());
                }

                if (selectedPosition == 1 || selectedPosition == 2) {
                    sessionManager.setIpAddress(enteredIp);
                }

                // Update spinner selection if the entered IP doesn't match any predefined option
                if (!enteredIp.equals(DEFAULT_IP) && !enteredIp.startsWith(ALTERNATE_IP) && selectedPosition != 2) {
                    ipAddressSpinner.setSelection(2); // Set to Custom
                }
            }
        });
    }
